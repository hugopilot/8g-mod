...
emoji_pattern = re.compile(
    ":([\w]*):|([\U0001F1E0-\U0001F1FF\U0001F300-\U0001F5FF\U0001F600-\U0001F64F\U0001F680-\U0001F6FF\U0001F700"
    "-\U0001F77F\U0001F780-\U0001F7FF\U0001F800-\U0001F8FF\U0001F900-\U0001F9FF\U0001FA00-\U0001FA6F\U0001FA70"
    "-\U0001FAFF\U00002702-\U000027B0])")
...
class AntiSpam(Cog):
    ...
    async def on_message(self, message: discord.Message) -> None:
        ...
        relevant_messages = [msg async for msg in
                             message.channel.history(after=earliest_relevant_at, oldest_first=False) if
                             not msg.author.bot]
        ...
        md = [msg for msg in messages_to_check if
              Counter(msg.author for msg in messages_to_check)[msg.author] > config.spamthreshold]
        if len(md) > 0:
            # There is spam there, create a dict with messages that have the same content but somehow got through
            # first antispam round and add it to the deletion queue
            scm = [msg for msg in messages_to_check if
                   Counter(msg.content for msg in messages_to_check)[msg.content] > 1 and
                   Counter(msg.author for msg in messages_to_check)[msg.author] > 1]
            if len(scm) > 0:
